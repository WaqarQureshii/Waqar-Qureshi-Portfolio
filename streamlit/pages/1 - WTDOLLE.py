...
# --- VIX Database Generation ---
    sidebar_counter += 1
    vix = Generate_DB().get_database('^VIX', input_start_date, input_end_date, input_interval)
...
# ------ VIX UI for VIX % Change Selection ------
        ...
        if subheader_comparator_vix == 'Greater than':
            ...
            col1.metric(label = f"VIX % > {selected_pct_change}", value = f'{boolean_vix} @ {vix.pctchg_str}')
        else:
            boolean_vix, sp500_intersection, nasdaq_intersection, rus2k_intersection = signal_pct_change_manual(vix.db, subheader_comparator_vix, selected_pct_change, vix.pctchg_int, sp500_intersection, nasdaq_intersection, rus2k_intersection)
            col1.metric(label = f"VIX % < {selected_pct_change}", value = f'{boolean_vix} @ {vix.pctchg_str}')
...
if header_show_hyg == True:
    sidebar_counter += 1
    hyg = Generate_DB().get_database('HYG', input_start_date, input_end_date, input_interval)
...
if header_show_rsp == True:
    ...
    if 'Moving Average (MA)' in rspMAorRSI and 'RSI (Relative Strength Index)' in rspMAorRSI:
        ...
        #---database generator---
        rsp = Generate_DB().get_database('RSP', input_start_date, input_end_date, input_interval, ma_length=rsp_ma_length, rsi_value=rsp_rsi_length)
        ...
        rsp = Generate_DB().get_database('RSP', input_start_date, input_end_date, input_interval, ma_length=rsp_ma_length)
        ...
        sidebar_counter += 1
        rsp = Generate_DB().get_database('RSP', input_start_date, input_end_date, input_interval, rsi_value=rsp_rsi_length)
...
if show_yieldcurve == True:
    ...
    yielddiff_comparator_selection = st.sidebar.radio('Choose Yield Curve comparator',
                                                       ['Diff greater than', 'Diff less than'],
                                                       index = 0)
    selected_yieldcurve_diff = float(st.sidebar.text_input("Input Yield Diff to compare",
                                                 0.5,
                                                 key = 'Yield Curve level')) #TODO use st.select_slider for a range instead possibly?
    ...
    if show_yield_option == '30-year':
        ...
        if yielddiff_comparator_selection == "Diff greater than":
            yielddiff.calc_diffcomparison(selected_yieldcurve_diff, yielddiff_comparator_selection, sp500_intersection, nasdaq_intersection, rus2k_intersection)
            sp500_intersection, nasdaq_intersection, rus2k_intersection = yielddiff.sp500intersection, yielddiff.ndxintersection, yielddiff.rus2kintersection #TODO Latest Update

            col5.metric(label=f'Yield Diff > {selected_yieldcurve_diff}', value = f'{yielddiff.diff_boolean} @ {yielddiff.curr_yielddiff}')
...
with col1.expander("S&P500 Parameter Selection"):
    ...
    with sp500col2:
        ...
        if sp500rsishow:
            ...
            sp500 = Generate_DB().get_database('^GSPC', input_start_date, input_end_date, input_interval, sp500_rsi_length)
            ...
            sp500 = Generate_DB().get_database('^GSPC', input_start_date, input_end_date, input_interval)
...
with col2.expander("Nasdaq Parameter Section"):
    ...
    with ndxcol2:
        ...
        if ndxrsishow:
            ...
            ndx = Generate_DB().get_database('^IXIC', input_start_date, input_end_date, input_interval, ndx_rsi_length)
            ...
            ndx = Generate_DB().get_database('^IXIC', input_start_date, input_end_date, input_interval)
...
with col3.expander("Russell 2000 Parameter Section"):
    ...
    with rus2kcol2:
        ...
        if rus2krsishow:
            ...
            rus2k = Generate_DB().get_database('^RUT', input_start_date, input_end_date, input_interval, rus2k_rsi_length)
            ...
            rus2k = Generate_DB().get_database('^RUT', input_start_date, input_end_date, input_interval)
